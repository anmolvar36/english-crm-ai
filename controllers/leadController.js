  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.error('Auto-Project creation failed:', autoProjErr.message);
      }

      await connection.commit();
    } catch (convErr) {
      await connection.rollback();
      console.error('Auto-convert failed in database execution:', convErr.message);
    } finally {
      connection.release();
    }
  } catch (settingsErr) {
    console.error('Failed to process auto-convert settings:', settingsErr.message);
  }
};
...
const update = async (req, res) => {
  try {
    ...
    const [leads] = await pool.execute(
      `SELECT id, status FROM leads WHERE id = ? AND company_id = ? AND is_deleted = 0`,
      [id, companyId]
    );
    ...
    if (updates.length > 0) {
      ...
      // AI & Automation: If status changed to Won, trigger auto-conversion
      if (updateFields.status === 'Won' && oldStatus !== 'Won') {
        await triggerAutoConversion(id, companyId);
      }
    }
...
const updateStatus = async (req, res) => {
  try {
    ...
    // AI & Automation: If status is set to Won, trigger auto-conversion
    if (status === 'Won') {
      await triggerAutoConversion(id, companyId);
    }
